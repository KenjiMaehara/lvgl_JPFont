...
void create_keypad_screen(void) {
    ...
    if (screen2 == nullptr) {
        screen2 = lv_obj_create(NULL);  // スクリーンを作成

        // 時刻表示用のラベルを作成
        time_label_keypad = lv_label_create(screen2);
        lv_obj_align(time_label_keypad, LV_ALIGN_TOP_MID, 0, 5); // 例: 画面の上中央に配置
        lv_label_set_text(time_label_keypad, "00:00"); // 初期テキスト

        wifi_label_tenkey = lv_label_create(screen2);
        ...
        // 数字を表示するラベルを作成
        number_label = lv_label_create(screen2);
        ...
        lv_obj_t * btn1 = lv_btn_create(screen2); // ボタンを作成
